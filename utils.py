...
def eval_perc_queries_spa_sem_lift(test_queries, enc_dec, batch_size=1000, hard_negatives=False, eval_detail_log=False, do_spa_sem_lift=False):
    '''
    Given a list of queries, run enc_dec, compute average percentiel rank (APR) score with the negative samples and ground truth labels
    Args:
        test_queries: a dict()
...
def get_intersection_attention(out_dims, inter_decoder_atten_type, inter_decoder_atten_num=0,
                               inter_decoder_atten_act="leakyrelu", inter_decoder_atten_f_act='sigmoid'):
    '''
    ...
    if inter_decoder_atten_num == 0:
        ...
        if inter_decoder_atten_type == "concat":
            attn = IntersectConcatAttention(out_dims, out_dims, inter_decoder_atten_num,
                                            activation=inter_decoder_atten_act, f_activation=inter_decoder_atten_f_act,
                                            layernorm=False, use_post_mat=False)
        elif inter_decoder_atten_type == "concat_norm":
            attn = IntersectConcatAttention(out_dims, out_dims, inter_decoder_atten_num,
                                            activation=inter_decoder_atten_act, f_activation=inter_decoder_atten_f_act,
                                            layernorm=True, use_post_mat=False)
        elif inter_decoder_atten_type == "concat_postm":
            attn = IntersectConcatAttention(out_dims, out_dims, inter_decoder_atten_num,
                                            activation=inter_decoder_atten_act, f_activation=inter_decoder_atten_f_act,
                                            layernorm=False, use_post_mat=True)
        elif inter_decoder_atten_type == "concat_norm_postm":
            attn = IntersectConcatAttention(out_dims, out_dims, inter_decoder_atten_num,
                                            activation=inter_decoder_atten_act, f_activation=inter_decoder_atten_f_act,
                                            layernorm=True, use_post_mat=True)
        elif inter_decoder_atten_type == "dotproduct_scaled":
            attn = IntersectDotProductAttention(out_dims, out_dims, inter_decoder_atten_num, dotproduct_scaled=True,
                                                layernorm=False, use_post_mat=False)
        elif inter_decoder_atten_type == "dotproduct":
            attn = IntersectDotProductAttention(out_dims, out_dims, inter_decoder_atten_num, dotproduct_scaled=False,
                                                layernorm=False, use_post_mat=False)
