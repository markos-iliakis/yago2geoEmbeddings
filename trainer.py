...
from utils import get_metapath_decoder, get_intersection_decoder, get_intersection_attention, \
    get_encoder, get_spa_encoder
...
def make_args_combine(args):
    ...
    #         data=args.data_path.strip().split("/")[-2],
    #         args_task = args_task,
    #         depth=args.depth,
...
class Trainer:
    ...
    def __init__(self, args, graph, feature_modules, node_maps, out_dims, console=True):
        """
        Args:
        ...
        if args.task == "qa":
            ...
            self.enc_dec = QueryEncoderDecoder(graph, self.enc, self.dec, self.inter_dec, self.inter_attn,
                                               args.use_inter_node)

        self.enc_dec.to(args.device)

        if args.opt == "sgd":
            self.optimizer = optim.SGD(filter(lambda p: p.requires_grad, self.enc_dec.parameters()), lr=args.lr,
                                       momentum=0)
        elif args.opt == "adam":
    ...
    def load_edge_data(self, load_geo_query=False, test_query_keep_graph=False):
        ...
        train_queries = load_queries_by_formula(self.args.data_path + "/train_edges{:s}.pkl".format(file_postfix))
        print("Loading validation edge  data..")
        val_queries = load_test_queries_by_formula(self.args.data_path + "/val_edges{:s}.pkl".format(file_postfix),
                                                   keep_graph=test_query_keep_graph)
        print("Loading testing edge data..")
        test_queries = load_test_queries_by_formula(self.args.data_path + "/test_edges{:s}.pkl".format(file_postfix),
                                                    keep_graph=test_query_keep_graph)
    ...
    def eval_model(self, flag="TEST"):
        ...
        if self.args.eval_general:
            if self.args.eval_log:
                v, aprs, qtype2fm_auc, qtype2fm_q_prec = run_eval(self.enc_dec, queries, 0, self.logger,
        ...
        if self.args.geo_train:
            if self.args.eval_geo_log:
                v_geo, aprs_geo, qtype2fm_auc_geo, qtype2fm_q_prec_geo = run_eval(self.enc_dec, queries_geo, 0,
                                                                                  self.logger, geo_train=True,
                                                                                  eval_detail_log=True)
